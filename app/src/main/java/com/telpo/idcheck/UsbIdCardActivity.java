...
public class UsbIdCardActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		getWindow().setFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON,
				WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);//保持屏幕不熄屏
		setContentView(R.layout.idcard_usb);
		Log.d("idcard demo", "onCreate");
	...
	private class GetIDInfoTask extends AsyncTask<Void, Integer, Boolean> {
		...
		protected Boolean doInBackground(Void... arg0) {//in thread
			...
			if(SystemUtil.getDeviceType() == StringUtil.DeviceModelEnum.TPS900.ordinal()) {
				...
				IdCard idCard = null;
				idCard = new IdCard(UsbIdCardActivity.this);
				try {
					//IdCard.open(IdCard.IDREADER_TYPE_USB,UsbIdCardActivity.this);
					//info = IdCard.checkIdCard(1000);
					info = idCard.checkIdCardOverseas();
					Log.d("idcard demo", "decode info------------");
				...
				if (info != null) {
					...
					try {
						...
						Log.d("idcard demo", "decode image------------");
						bitmap = idCard.decodeIdCardImageOverseas(image,"/sdcard/Download/zp.bmp");
						Log.d("idcard demo", "decode bitmap-------------");
						if(bitmap == null){
							Log.d("idcard demo", "bitmap is null");
						}
					} catch (TelpoException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
		...
		protected void onPostExecute(Boolean result) {//after thread
			...
			if(SystemUtil.getDeviceType() == StringUtil.DeviceModelEnum.TPS900.ordinal() && info.getName().contains("timeout")) {
				...
				if (result) {
					...
					if(isCellPhone) {
						if (SystemUtil.getDeviceType() != StringUtil.DeviceModelEnum.TPS900.ordinal() && "I".equals(info.getCard_type())) {
							...
							if(info.getNation() != null) {
								cellphone_showData.append(getString(R.string.idcard_mz) + info.getNation() + "\n");
							}
							cellphone_showData.append(getString(R.string.idcard_csrq) + info.getBorn() + "\n"
									+ getString(R.string.idcard_dz) + info.getAddress() + "\n"
									+ getString(R.string.idcard_sfhm) + info.getNo() + "\n");
							if(info.getPassNum() != null) {
								cellphone_showData.append(getString(R.string.idcard_txzhm) + info.getPassNum() + "\n");
							}
							if(info.getIssuesNum() != null) {
								cellphone_showData.append(getString(R.string.idcard_qfcs) + info.getIssuesNum() + "\n");
							}
							
							cellphone_showData.append(getString(R.string.idcard_qzjg) + info.getApartment() + "\n"
											+ getString(R.string.idcard_yxqx) + info.getPeriod());
						...
						if (SystemUtil.getDeviceType() != StringUtil.DeviceModelEnum.TPS900.ordinal() && "I".equals(info.getCard_type())) {
							...
							idcardInfo.append(getString(R.string.idcard_csrq) + info.getBorn() + "\n"
									+ getString(R.string.idcard_dz) + info.getAddress() + "\n"
									+ getString(R.string.idcard_sfhm) + info.getNo() + "\n");
							if(info.getPassNum() != null) {
					...
					Log.d("idcard demo", "failed count");
					failedCount++;
					if (isCircle) {
						isCount = false;
		...
		public void onUsbPermissionEvent(UsbDevice dev, boolean granted) {
			if (granted) {
				try {
					...
					if (sdta == null) {
						//Toast.makeText(UsbIdCardActivity.this, "妯″潡杩炴帴澶辫触", Toast.LENGTH_SHORT).show();
					} else {
						//Toast.makeText(UsbIdCardActivity.this, "妯″潡杩炴帴鎴愬姛", Toast.LENGTH_SHORT).show();
					}
				}
			}
		}
    ...
    public void initVoice2(){
        soundPool2= new SoundPool(10, AudioManager.STREAM_MUSIC, 3);//绗竴涓弬鏁颁负鍚屾椂鎾斁鏁版嵁娴佺殑鏈�澶т釜鏁帮紝绗簩鏁版嵁娴佺被鍨嬶紝绗笁涓哄０闊宠川閲�
        music = soundPool2.load(UsbIdCardActivity.this, R.raw.beep, 1); //鎶婁綘鐨勫０闊崇礌鏉愭斁鍒皉es/raw閲岋紝绗�2涓弬鏁板嵆涓鸿祫婧愭枃浠讹紝绗�3涓负闊充箰鐨勪紭鍏堢骇
    }
