...
public class UsbTACardActivity extends Activity{
	// 8932AB6C1D37 // = { (byte) (137&0xff), (byte) (50&0xff), (byte) (171&0xff), (byte) (108&0xff), (byte) (29&0xff), (byte) (55&0xff)};
	private byte[] Password; //= {137, 50, 171, 108, 29, 55};
	...
	private Handler handler3 = new Handler() {
		...
		public void handleMessage(Message msg) {
		    ...
		    case 4:
				...
				if(usbTACard == null) {
					try {
						usbTACard = new UsbTypeA(tUsbManager, tcard_reader);
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		read_again.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(readCircle) {
					read_again.setText("停止循环读卡");
				}else {
					read_again.setText("开始循环读卡");
		...
		//read_ta_circle.setVisibility(View.GONE);
		...
		while (iterator.hasNext()) {
			...
			if ((tpid == IdCard.READER_PID_SMALL && tvid == IdCard.READER_VID_SMALL) 
				...
				if (tUsbManager.hasPermission(tusbDevice)){
					...
					tUsbManager.requestPermission(tusbDevice, tPermissionIntent);//***寮瑰嚭瀵硅瘽妗嗛棶鏄惁鍏佽鐨勬椂锟�?
				}
			}
		}
		isUsingtUsb = false;
	...
	public void openLockCheck() {
		...
		if (hastReader && tcard_reader != null ) {
			Log.e("lichao", "Password=" + Arrays.toString(Password));
	...
	private class PwCheckTask extends AsyncTask<Void, Integer, Boolean>{
		...
		protected Boolean doInBackground(Void...  arg1) {
			...
			try {
				pwright = usbTACard.checkPW();
			} catch (Exception e) {
				Log.e("lichao", "Exception=" + e.getMessage());
				return false;
			}
			Log.e("lichao", "pwright=" + pwright);
			return pwright;				
		}
	...
	private class GetTAInfoTask extends AsyncTask<Void, Integer, Boolean>{
		...
		protected void onPreExecute() {
			...
			Log.d("idcard demo", "startTime:"+startTime);
		...
		protected void onPostExecute(Boolean result){
			try {
				...
				if (result) {
					//success_count++;
					Log.d("idcard demo", "success endTime:"+(System.currentTimeMillis()-startTime));
					tcardInfo.setText(getString(R.string.idcard_kh) + tinfo.getNum());
					tcardInfo.setVisibility(View.VISIBLE);
					showtInfoView();
					...
					Log.d("idcard demo", "failed endTime:"+(System.currentTimeMillis()-startTime));
					pwCheck.setVisibility(View.GONE);
					dataWrite.setVisibility(View.GONE);
					tgetPermission.setVisibility(View.GONE);
					pwConfirm.setVisibility(View.GONE);
					...
					tdef_view.setVisibility(View.GONE);
					tcardInfo.setVisibility(View.GONE);
					handler3.sendEmptyMessageDelayed(1, 100);
					return;
				}
				...
				//count_text.setText("鎬绘鏁帮細"+count_time+"  鎴愬姛娆℃暟锛�"+success_count+"  澶辫触娆℃暟锛�"+(count_time-success_count));
			} catch (Exception e) {
				// TODO: handle exception
				e.printStackTrace();
			}
